# Hi, I'm Jeremiah Fiifi Tetteh 👋
### AI & IoT Solutions Engineer · Peer Reviewer · MathWorks Contributor
📍 Accra, Ghana · Open to relocation globally

[![Portfolio](https://img.shields.io/badge/Portfolio-jeremiahtetteh.github.io-00875a?style=flat-square&logo=github)](https://jeremiahtetteh.github.io)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-jeremiah--tetteh-0077B5?style=flat-square&logo=linkedin)](https://linkedin.com/in/jeremiah-tetteh)
[![Email](https://img.shields.io/badge/Email-tettehjeremiah%40ymail.com-D14836?style=flat-square&logo=gmail)](mailto:tettehjeremiah@ymail.com)

---

## 🧠 About Me

I build AI systems that solve real problems — from waste segregation to gesture-controlled robotics — with a focus on impact in Africa and emerging markets.

- 🏫 **Engineering Research Assistant & Lecturer** at Ghana Communication Technology University (GCTU) · Jan 2026–Present
- 🔬 **Peer Reviewer** across 5 top-tier journals — Elsevier, Wiley, World Scientific, Inderscience & Emerald Publishing · 10 reviews for *Expert Systems with Applications* · **Certificate of Reviewing**
- 🎓 **MathWorks Computer Vision Engineer** — 9-course professional certificate (Coursera, April 2026)
- 🤝 **MathWorks UX Contributor** — Shaping Generative AI in MATLAB Help Center since 2021
- 💚 **IoT & Embedded Systems Engineer** at HIVE Consult & Edenway Foundation
- 🛠️ Self-balancing bicopter, IoT automation, PID control, ESP32/Arduino

---

## 🚀 Featured Projects

| Project | Tech | Result |
|---|---|---|
| 🗑️ Solid Waste Segregation CNN | ResNet-101 · MATLAB · Raspberry Pi | 98–100% accuracy · Published |
| ✋ Hand Gesture Recognition | YOLOv4-Tiny · MobileNet · MATLAB R2025b | 5 gestures · Real-time |
| 🚁 Self-Balancing Bicopter | Arduino · MPU6050 · PID | ±2° stability · Working prototype |
| 🌿 Smart Cocoa Sorting | DenseNet-201 · MATLAB | 98%+ accuracy |
| 🪵 Wood Knots Detector | ACF Detector · MATLAB R2025b | 9.1% miss rate |
| 🔊 Machine Fault Detection | YamNet CNN · Acoustic ML | Real-time anomaly detection |

---

## 🛠️ Skills & Tools

![MATLAB](https://img.shields.io/badge/MATLAB-R2025b-e16737?style=flat-square)
![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white)
![TensorFlow](https://img.shields.io/badge/TensorFlow-FF6F00?style=flat-square&logo=tensorflow&logoColor=white)
![Arduino](https://img.shields.io/badge/Arduino-00979D?style=flat-square&logo=arduino&logoColor=white)
![ESP32](https://img.shields.io/badge/ESP32-IoT-black?style=flat-square)
![HTML](https://img.shields.io/badge/HTML-E34F26?style=flat-square&logo=html5&logoColor=white)

**Domains:** Computer Vision · Deep Learning · Embedded Systems · IoT · Academic Peer Review · UAV/Drones

---

## 📜 Certifications

- 🏅 **MathWorks Computer Vision Engineer** — Coursera, April 2026
- 🧬 **Neural Networks & Deep Learning** — DeepLearning.AI (Andrew Ng)
- 🤖 **Machine Learning Specialization** — DeepLearning.AI (Andrew Ng)
- 🏛️ **Accelerated CS Fundamentals** — University of Illinois Urbana-Champaign
- ✈️ **FAA Part 107 Remote Pilot** — Drone Operations
- 📋 **ISO 9001:2015** — Quality Management Systems

---

## 📊 GitHub Stats

![Jeremiah's GitHub Stats](https://github-readme-stats.vercel.app/api?username=bigjerryfisher&show_icons=true&theme=default&hide_border=true&color=00875a)
![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=bigjerryfisher&layout=compact&hide_border=true&theme=default)

---

*"Translating mentorship into momentum, collaboration into capability, and tools into tangible change."*
